...
RPMFLAGS=--ba
SRCRPM=parsegraph_server_config-1-1.src.rpm
SPECFILE=parsegraph_server_config.spec

make dist-gzip
cp -u parsegraph_server_config-1.tar.gz $RPMDIR/SOURCES
...
[ -L ../rpm/$SRCRPM ] || ln -v -s $RPMDIR/SRPMS/parsegraph_server_config-1-1`rpmbuild -E '%{?dist}' $SPECFILE`.src.rpm ../rpm/$SRCRPM
